class FamiliaDatabase:
    ...
    def get_pai_de_gabriel(self):
        query = "MATCH (p1:Pessoa)-[:PAI_DE]->(p2:Pessoa {nome: 'Gabriel Guimaraes'}) RETURN p1.nome AS nome_pai"
        results = self.db.execute_query(query)
        return [result["nome_pai"] for result in results]

    # Com quem Lise é casada?
    def get_casados_com_lise(self):
        query = "MATCH (p1:Pessoa {nome: 'Lise'})-[:CASADO_COM]->(p2:Pessoa) RETURN p2.nome AS nome_parceiro"
        results = self.db.execute_query(query)
